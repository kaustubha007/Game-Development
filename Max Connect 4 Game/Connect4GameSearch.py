...
class maxConnect4Game:
    ...
    # The AI section.
    def aiPlay(self):
        searchTree = DepthLimitedSearch(self, self.maxDepth)
        move = searchTree.makeMove()
        result = self.playPiece(move)
        print('\n\nMove %d: Player %d, column %d\n' % (self.pieceCount, self.currentTurn, move+1))
...
#Function to get all the possible moves for the provided board state.
def getAllMoves(gameBoard):
	allMoves = []
	for column, value in enumerate(gameBoard[0]):
		if value == 0:
			allMoves.append(column)
	return allMoves

#Function to create a new game board which is further used for exploring the search tree.
#Basically it creates a new state of the tree which is then used to make next move.
...
#A class that implements the depth limited search with alpha beta pruning to extract the next best move to be made.
class DepthLimitedSearch:
	def __init__(self, currentGame, depth):
		self.currentTurn = currentGame.currentTurn
		self.currGame = currentGame
		self.maxDepth = int(depth)
	#Function that returns the next move that the computer can make.
	def makeMove(self):
		minVals = []
		allMoves = getAllMoves(self.currGame.gameBoard)
                
		for move in allMoves:
			nextGame = createGameforSearch(self.currGame, move)
			minVals.append(self.minPlayer(nextGame, -99999, 99999))
			
		selectedMove = allMoves[minVals.index(max(minVals))]
		
		return selectedMove

	def minPlayer(self, gameState, alpha, beta):
		if gameState.pieceCount == 42 or gameState.nodeDepth == self.maxDepth:
                    return self.getUtilityValue(gameState)
		v = 99999
                for move in getAllMoves(gameState.gameBoard):
			newGameState = createGameforSearch(gameState,move)
			v = min(v, self.maxPlayer(newGameState, alpha, beta))
			if v <= alpha:
				return v
	...
	def maxPlayer(self, gameState, alpha, beta):
		if gameState.pieceCount == 42 or gameState.nodeDepth == self.maxDepth:
                    return self.getUtilityValue(gameState)
		v = -99999
		for move in getAllMoves(gameState.gameBoard):
			newGameState = createGameforSearch(gameState, move)
			v = max(v,self.minPlayer(newGameState, alpha, beta))
			if v >= beta:
				return v
	...
	def getUtilityValue(self, gameState):
		if self.currentTurn == 1:
			utilityValue = gameState.player1Score * 2 - gameState.player2Score
		elif self.currentTurn == 2:
			utilityValue = gameState.player2Score * 2 - gameState.player1Score
